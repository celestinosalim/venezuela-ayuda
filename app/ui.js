...
export default function Ui({ seismic, health, updatedAt }) {
  ...
  function Stamp({ items, field = "u" }) {
    ...
    return (
      <div className="vstamp ok">
        <span className="live" />✓ {t.verified}{since ? ` · ${since}` : ""}
      </div>
    );
  }
  ...
  return (
    <>
      ...
      <div className="wrap">
        {/* SEISMIC (live USGS, refreshed every 15 min) */}
        <section>
          ...
          <div className="quakebox">
            {seismic.error || !seismic.quakes.length ? (
              ...
              <>
                ...
                <div className="stamp">
                  <span className="live" />{t.updated}{since ? ` · ${since}` : ""}
                </div>
              </>
            )}
          </div>
        ...
        {/* DROP-OFF (real locations + official source, re-checked every 15 min) */}
        <section>
          <h2 id="entregar">{t.entregar[0]}</h2>
          <div className="sub">{t.entregar[1]}</div>
          <Stamp items={DROP} field="src" />
          <div className="grid">{DROP.map((o, i) => <DropCard key={i} o={o} />)}</div>
